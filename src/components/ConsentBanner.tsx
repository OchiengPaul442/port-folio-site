...
export function ConsentBanner() {
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener('open-privacy-settings', openSettings);
  ...
  const choose = (value: 'granted' | 'denied') => {
    ...
    setChoice(value);
    setFeedback(value === 'granted'
      ? 'Analytics enabled. Thanks for supporting this portfolio.'
      : 'No problem. Optional analytics will stay off.');
  ...
  return (
    <aside
      ...
      className={`consent-panel fixed inset-x-4 bottom-4 z-[60] mx-auto max-w-2xl rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-secondary)]/95 p-5 shadow-2xl shadow-black/30 backdrop-blur-xl sm:inset-x-6 sm:p-6 ${feedback ? 'consent-panel-confirmed' : ''}`}
    >
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="relative min-w-0">
          <span className="consent-orb" aria-hidden="true" />
          {feedback ? (
